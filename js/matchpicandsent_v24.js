(function(){
	...
	// PlayerScene 3 works now. The falling words with a THUD has been included.  June 18
    // Added Xpos array and keyboard graphic for player to input answers by typing   June 25
	// More refining the keyboard touch      July 2
	// Can read the first row keys:  q w e r t y   July 2
	// Now time to BONK when player inputs the wrong text at the keyboard stage   july 10
...
(function () {
  ...
  // Some arrays and variables
    ...
    var kBrdAns = ["The cat is going into the house.", "The cat has gone into the house.", "The cat went into the house.", "The cat went into the house last week."];
	...
	var FINwordsX = [[39,0],[99,0],[159,0],[219,0],[279,0],[339,0],[399,0],[459,0],[519,0],[579,0],[639,0]];
	var FINwordsXORG = [[39,0],[99,0],[159,0],[219,0],[279,0],[339,0],[399,0],[459,0],[519,0],[579,0],[639,0]];
	var keysLocsXY = [[,],[,],[,]];
	var startY = [[52,72], [89, 110], [126, 148],[162, 185]]
	...
	var keyBrdScene1 = false;
	var keyBrdScene2 = false;
	var keyBrdScene3 = false;
	var keyboardChars = 
	[
	     "qwertyuiop",
		 " asdfghjkl",
	...
	var keyBOutput = 10;  var keyBstring = ""; var posX = 55; var lastChar;
	var newX;
	
	// set the X coords for the keyboard keys ///////////////////////////////////////
  	   var rowStartX = 326;
	   var rowShift;
 	   var startX;
	   
	   for (var row = 0; row < keyboardChars.length; row++)
	   {
		 newX = 0;
		 startX = rowStartX + 12; // the q key is at 326 + 12 pixels. 
		 rowStartX = startX;
		 rowShift = 0;
		 
			for (var col = 0; col < keyboardChars[row].length; col++)
			{
				newX = startX + rowShift;
				startX =  newX;
				rowShift = 26;
				keysLocsXY[row][col] = newX;
				console.log ("keys is row and col  " + row + "  " + col + "  " + keysLocsXY[row][col]);
			}
 	   }
	   /////////////////////////////////////  end  //////////////////////////////////
	
	var natureBG = createSprite();
	natureBG.Id = 0;
	natureBG.srcX = 1;
	natureBG.srcY = 1;
	natureBG.srcWdth = 799;
	natureBG.srcHigh = 570;
	natureBG.width = 300;    
	natureBG.height = 190;     
	natureBG.x = 20;
	natureBG.y = 8;;
	natureBG.visible = true;
 	sprites.push(natureBG);
    
	var keyboard = createSprite();
	keyboard.Id = 100;
	keyboard.srcX = 8;
	keyboard.srcY = 1769;
	keyboard.srcWdth = 474;
	keyboard.srcHigh = 229;
	keyboard.width = 320;    
	keyboard.height = 179;        
	keyboard.x = 333;
	keyboard.y = 35;
	keyboard.visible = false;
 	sprites.push(keyboard);
	...
	plyrUnderlineFIN.srcHigh = 29;
	plyrUnderlineFIN.width = 470;    
	plyrUnderlineFIN.height = 16;     
	plyrUnderlineFIN.x = 28;
	plyrUnderlineFIN.y = 229;
	...
	image.src = "./imgs/tilesheets/tilesheet14.png";
	...
	function touchstartHandler(event)
	{
	   ...
	   {
 		   if ((touchX > plyrTheFIN.x && (touchX < plyrTheFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrCatFIN.x && (touchX < plyrCatFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrIsgoingFIN.x && (touchX < plyrIsgoingFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrGoesFIN.x && (touchX < plyrGoesFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrHasgoneFIN.x && (touchX < plyrHasgoneFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrWentFIN.x && (touchX < plyrWentFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrIntoFIN.x && (touchX < plyrIntoFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrthe2FIN.x && (touchX < plyrthe2FIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrHouseFIN.x && (touchX < plyrHouseFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
	   ...
	   {
 		   if ((touchX > plyrLstWeekFIN.x && (touchX < plyrLstWeekFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
       ...
       if (plyrPeriod.enabled) 
	   {
		   ...
		   {
	           let wordlim;
               if (plyrScene3.visible) { wordlim = 8; } else { wordlim = 7; }			   
		       if (wrdCntr < wordlim) 
			   {
				   ding.play();
				   plyrPeriod.enabled = false;
			   ...
			   {
 				   alert ("You cannot have more than " + wordlim + " words in the answer sentence");
			   }
		   }
	   }
	   if (plyrPeriodFIN.enabled) 
	   {
 		   if ((touchX > plyrPeriodFIN.x && (touchX < plyrPeriodFIN.x + 58)) && (touchY > 220 && touchY < (243)))
		   { 
 	   ...
 	   if (lastOKbtn.enabled) 
	   {
		   ...
		   {      
                if (plyrScene1.visible)
				{
					...
					{ 
				      console.log ("Success 222222");
					   ...
					   plyrScene3.visible = true;
					 		if (wordThudInc > 0) { wordThudReset() ; }
                    }
			        else 
					{
  					   grrr.play();
				  ...
				  else if (plyrScene3.visible)
				 {
					...
					let Id;
					let ansIdstr = " ";
 					const idString1 = " 150 151 155 156 157 158 159 160 0 0 0 ";
			       ...
			       else 
				   {
					   ...
					   if (!keyBrdScene1) { clearScreenforTyping(); }
					   clearFINwords();
					   var sourceClone = FINwordsXORG.map(row => [...row]);
					   FINwordsX.splice(0, FINwordsX.length, ...sourceClone);					
					   wrdCntr = 0;
		...
		if (keyBrdScene1) 
		{
			...
			let breakFlg = 0;
			let playStrLen;
			let correctAnsStrLen;
    
		 	console.log ("touchX and touchY are .................... " + touchX + "    " + touchY);
 		    let intKeyLocXYlow;
			let intKeyLocXYhi;
 			let keybrow;
			let ybound1, ybound2;	 
	    	if ((touchX > keyboard.x && (touchX < keyboard.x + 275)) && (touchY > keyboard.y && touchY < (keyboard.y + 150 )))
            {  
				 	for (var r = 0; r < startY.length; r++)
					{
 						ybound1 = parseInt(startY[r][0]);
 						ybound2 = parseInt(startY[r][1]);
 						
						if (touchY >= ybound1 && touchY <= ybound2)
						{
		...
		// 	kBrdAns[0]
         			...
         			for (var col = 0; col < keyboardChars[keybrow].length; col++)
					{
						intKeyLocXYlow =  parseInt(keysLocsXY[keybrow][col],10);
					 	intKeyLocXYhi = intKeyLocXYlow + 26;
						
					 	if ( touchX >= intKeyLocXYlow && touchX <= intKeyLocXYhi )
						{
							keyNo = keybrow + col;
 						    renderTxt = 2;
						    sceneNum = keyBOutput;
							keyBstring = keyBstring  + keyboardChars[keybrow][keyNo];
 					    	if (keyBstring.length == 1)  // Capitalize the first letter so 'the'  -> 'The'
							{  
                                keyBstring = keyBstring.charAt(0).toUpperCase();							
						    }  
							lastChar = keyBstring.at(-1); 
							if (lastChar == ".") 
							{  
						        squak.play(); 
							}
                            ...
                            dotAnswers[sceneNum] = 	keyBstring;						
							breakFlg = 1;
						}
						if (breakFlg) { break; }				   
					}
  ...
  // ***********************************************************************************
	...
	function wordThudReset()
	{
		 plyrCat.y -= wordThudInc;
 		 plyrGoes.y -= wordThudInc;
 		 plyrHasgone.y -= wordThudInc;
 		 plyrHouse.y -= wordThudInc;
 		 plyrInto.y -= wordThudInc;
         plyrIsgoing.y -= wordThudInc;
 		 plyrLstWeek.y -= wordThudInc;
 		 plyrPeriod.y -= wordThudInc;
         plyrThe.y -= wordThudInc;
         plyrthe2.y -= wordThudInc;
         plyrWent.y -= wordThudInc;
   		 plyrScene1.y -= wordThudInc;
	}			
	...
	function render(textOut, dotAns, dtIndx)
	{
		...
		drawingSurface.clearRect(0,0,canvas.width, canvas.height);
    		...
    		if (textOut == 2 && dotAns[dtIndx] !== "" ) 
			{     
  		         let posY = 200;
				 drawingSurface.fillStyle = "black"; // Background color
				 drawingSurface.fillRect(47, posY, 269, 30);
				 drawingSurface.fillStyle = 'white';
				 drawingSurface.font = "20px Helvetica";  console.log ("dotAns[c] is        " + dotAns[dtIndx]);
				 drawingSurface.fillText(dotAns[dtIndx], posX, posY + 20); 
