...
(function () {
  ...
  // Some arrays and variables
	...
	var keys = [[,],[,],[,],[,],[,],[,],[,],[,],[,],[,]]; 
 	var startY = [51.6,69.2,99.05,129.0 ];
	const initXpos = 339;
 	const keyH = 35;
	var gap = 3;
	var newX = 0;
    ...
    // assign x coords to keys' locations
	// there are 3 Y values. So keychararray[0 - 9] = 1785Y, keychararray [10 - 18] = 1822Y and keyarray[19 - 27] = 1857Y
    for (var row = 0; row < keyboardChars.length; row++)
	{
		var startX = initXpos + addX[row];
		const keyW = 35;
 		for (var col = 0; col < keyboardChars[row].length; col++)
		{
		    newX = startX + (col*(keyW + gap));
  		    keys[row][col] = newX;
			console.log ("keys is row and col  " + row + "  " + col + "  " + keys[row][col]);
     	}
	}
	...
	keyboard.width = 320;    
	keyboard.height = 179;         //  y is 60 92  119 
	keyboard.x = 323;
	keyboard.y = 35;
	...
	image.src = "./imgs/tilesheets/tilesheet12.png";
	...
	function touchstartHandler(event)
	{
 	   ...
 	   if (lastOKbtn.enabled) 
	   {
		...
		if (keyBrdScene1) 
		{
			console.log ("found the keyBrdScene1 place");
	    	/* // the KEYBOARD section 
	        if ((touchX > keyboard.x && (touchX < keyboard.x + 275)) && (touchY > keyboard.y && touchY < (keyboard.y + 150 )))
		    {      
	   		    console.log (" touching the keyboard>>>>>>>>>>>>>>");
				 for (var row = 0; row < keyboardChars.length; row++)
	             {
		             for (var col = 0; col < keyboardChars[row].length; col++)
					{
					    console.log ("keyboardchars row col are  "+ row + " " + col + " "  + keyboardChars[row][col]);
					}
	             }
	         } */
		...
		if ((touchX > keyboard.x && (touchX < keyboard.x + 275)) && (touchY > keyboard.y && touchY < (keyboard.y + 150 )))
        {         
	 // isolate the player's key choice Location
	      touchY += 0.05;
	      console.log ("touchY is ??????? " + touchY);
	 
           var tempArr = [];
		   tempArr = [...startY];
           tempArr.push(touchY);                  //51.6 - 69.2  - 99.05  - 129.0 
           tempArr.sort((a, b) => a - b);
		   
		   console.log("SORTED tempArr is >>>>>>>> " + tempArr);
		   
           var locY = tempArr.indexOf(touchY);
		   if (locY == 1)  { locY = 60; }
		   if (locY == 2)  { locY =  92; }
		   if (locY == 3)  { locY = 119; }
		   
		   console.log ("locY is ...............    " + locY);
...
// Creates a shallow copy of the elements
const targetArray = [...sourceArray]; 

console.log(targetArray); // Output: [5, 6, 7]
