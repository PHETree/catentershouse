 	   ...
 	   if (lastOKbtn.enabled) 
	   {
                ...
                if (plyrScene1.visible)
				{
				  ...
				  else if (plyrScene3.visible)
				 {
			       ...
			       else 
				   {
				       ...
				       lastOKbtn.visible = false;
					   lastOKbtn.enabled = false;
					console.log ("value of keyBrdScene1 is  " + keyBrdScene1);
		...
		if (keyBrdScene1) 
		{
            ...
            {  
				...
				{			
					if (breakFlg) { break; }	 */			   
		            // First determine which keyboard row the player is in. This is 
					// based on touchY
					
					for (var r = 0; r < startY.length; r++)
					{
						console.log ("inside the r loop thing*************************************");
						ybound1 = parseInt(startY[r][0]);
						console.log ("ybound1 is ????????????????????????????  " + ybound1);
						ybound2 = parseInt(startY[r][1]);
						console.log ("ybound2 is ????????????????????????????  " + ybound2);
         			...
         			for (var col = 0; col < keyboardChars[keybrow].length ; col++)
					{
						...
						console.log ("intKeyloc low is  row and col are  " + intKeyLocXYlow + "row: " + keybrow + " col: " + col);
						intKeyLocXYhi = intKeyLocXYlow + 26;
						
					 	if ( touchX >= intKeyLocXYlow && touchX <= intKeyLocXYhi )
						{
							keyNo = keybrow + col;
			console.log ("this is the pressed key letter (maybe) " + keyNo + " letter   : " + keyboardChars[keybrow][keyNo]);
							breakFlg = 1;
						}
						if (breakFlg) { break; }				   
					}
	 
      // touchY   51 to 73   88 to 110  127 to 147  space bar: 160 to 182
		     /*  } */
		  }
	   }
